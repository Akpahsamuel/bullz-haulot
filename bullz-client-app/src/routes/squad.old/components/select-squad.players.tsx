...
import { SquadForm } from "../../squad/types";
  ...
  const handlePlayerSelect = (token: TokenResponse) => {
    ...
    if (foundPlayer)
      playerArray.update(playerArrayWatch.indexOf(foundPlayer), {
        ...foundPlayer,
        name: token.coinAddress, // Use full token type string
            ...
            <>
              ...
              {priceList.map((token) => (
                <TokenCard
                  ...
                  imageUrl=""
                  onClick={() => handlePlayerSelect({ ...token, imageUrl: token.imageUrl || "" } as TokenResponse)}
                />
              ))}
            </>
          )}
